...
def test_db_groups_reproduce_ester_alcohol_azeotropes():
    """The bundled `unifac_groups` are only useful if they predict real VLE, so
    gate them on two literature azeotropes, straight off the DB records (Antoine
    + groups, no binary parameters anywhere):

        methyl acetate / methanol   x = 0.657, 53.8 C   (Gmehling, 760 mmHg)
        ethyl acetate / ethanol     x = 0.539, 71.8 C

    Bands are 0.05 in mole fraction and 1.5 K — group contribution, not a fit.
    """
    from core import component_db as db

    udb = load_unifac_db()
    for (a, b), (x_lit, T_lit) in {("methyl acetate", "methanol"): (0.657, 53.8),
                                   ("ethyl acetate", "ethanol"): (0.539, 71.8)}.items():
        ra, rb = db.get(a), db.get(b)
        fn = unifac_gamma_fn([ra["unifac_groups"], rb["unifac_groups"]], udb)
        antoine = np.array([ra["antoine"], rb["antoine"]])
        xs = np.linspace(0.002, 0.998, 400)
        Ts = np.array([bubble_T(np.array([x, 1 - x]), 760.0, antoine, gamma_fn=fn)
                       for x in xs])
        k = int(np.argmin(Ts))
        assert 0 < k < len(xs) - 1, f"{a}/{b}: no interior minimum-boiling point"
        assert abs(xs[k] - x_lit) < 0.05, (a, b, xs[k], x_lit)
        assert abs(Ts[k] - T_lit) < 1.5, (a, b, Ts[k], T_lit)
