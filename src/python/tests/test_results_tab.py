...
def test_active_comps_drops_all_zero():
    from gui.plotting import active_comps

    comps = ["a", "b", "c"]
    x = np.array([[0.6, 0.0, 0.4], [0.5, 0.0, 0.5]])   # comp "b" absent everywhere
    keep, kept = active_comps(x, comps)
    assert keep == [0, 2] and kept == ["a", "c"]


if __name__ == "__main__":
    test_active_comps_drops_all_zero()
