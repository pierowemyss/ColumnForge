...
Three actions:
  ...
  * Analyse       -- pinches and bodies at the chosen (r, E/F); is it feasible?
...
SECTION_COLOR = {"rectifying": "tab:blue", "extractive": "tab:purple",
                 "stripping": "tab:red", "intermediate": "tab:green"}
...
def _active_bodies(res):
    """{section name -> set of body indices taking part in a junction}.

    A set, and built from both members of every pair, because the middle section
    of an extractive column appears in TWO gaps and the body that reaches up to
    the rectifying section need not be the one that reaches down to the stripping
    section. Collapsing this to one index per section -- which a dict
    comprehension over `gaps` silently does, the second gap overwriting the first
    -- highlighted the wrong body at the upper junction.
    """
    out = {}
    for g in res["gaps"]:
        for name, idx in zip(g["pair"], g["active"]):
            if idx is not None:
                out.setdefault(name, set()).add(int(idx))
    return out


def _region_job(prob, provider, grid, r_hi, n_scan, EF,
    ...
    reg = _rbm_api.operating_region(
        prob, provider, EF_grid=grid, r_hi=r_hi, n_scan=n_scan,
        on_step=(None if report is None
                 else lambda done, total: report(done, total, 0.0)),
        cancelled=cancel)
    k = int(np.argmin(np.abs(np.asarray(reg["EF"], float) - float(EF))))
    lo, hi = float(reg["r_min"][k]), float(reg["r_max"][k])
    band = (None, None) if not np.isfinite(lo) else (
        lo, None if not np.isfinite(hi) or hi >= r_hi else hi)
...
class RBMModuleWidget(QWidget):
    ...
    def __init__(self, window_state=None, parent=None):
        ...
        self._result = None          # last `analyse` dict
    ...
    def _setup_ui(self):
        ...
        self.analyse_btn.setToolTip("Solve every section's pinches at this (r, E/F), "
        ...
        for b in (self.analyse_btn, self.band_btn, self.region_btn,
                  self.cancel_btn):
        ...
        self.view_combo.addItems(["Ternary (LK vs HK)", "Pinches & bodies",
                                  "Operating region"])
    ...
    def _sync_extractive_enabled(self, on):
        ...
        silently ignored)."""
        for w in (self.entrainer_combo, self.ef_spin, self.ef_lo, self.ef_hi,
                  self.ef_pts):
    ...
    def _refresh_species(self):
        ...
        for combo, default in ((self.lk_combo, 0), (self.hk_combo, 1),
    ...
    def _gather(self):
        ...
        self._thermo_note = note

        extractive = self.extractive.isChecked()
        x_E = None
        if extractive:
            if ent_stream is not None:
                x_E = np.array([ent_stream.composition.get(n, 0.0)
        ...
        prob = build_problem(
            ...
            x_E=x_E, extractive=extractive, **kw)
    ...
    def set_params(self, params: dict):
        names = self._species_order()
        for key, combo in (("lk", self.lk_combo), ("hk", self.hk_combo),
                           ("entrainer", self.entrainer_combo)):
            v = params.get(key)
            if v in names:
                combo.setCurrentIndex(names.index(v))
        for key, spin in (("rec_lk", self.rec_lk), ("rec_hk", self.rec_hk),
                          ("q_spin", self.q_spin), ("r_spin", self.r_spin),
                          ("ef_spin", self.ef_spin), ("r_hi", self.r_hi),
                          ("ef_lo", self.ef_lo), ("ef_hi", self.ef_hi)):
            if key in params:
    ...
    def _run_bg(self, label, job, on_done):
        ...
        buttons = (self.analyse_btn, self.band_btn, self.region_btn)
        for b in buttons:
            b.setEnabled(False)
    ...
    def _on_analyse(self):
        ...
        self._run_bg("Analysing",
                     lambda: _rbm_api.analyse(prob, provider, r=r, EF=EF),
                     self._on_analyse_done)

    def _on_analyse_done(self, result):
        self._result = result
        ...
        n_c = sum(p.get("clipped", False)
                  for s in result["sections"] for p in s["pinches"])
        resid = result.get("balance_residual", 0.0)
        gaps = ", ".join(f"{a}/{b} {g['distance']:.4g}"
        ...
        self.status.setText(
            ...
            + (f"  WARNING: products leave {resid:.3g} of the feed unaccounted "
               f"for; D was fit by least squares." if resid > 1e-3 else "")
    ...
    def _on_band(self):
        ...
        if self.extractive.isChecked():
            grid = np.linspace(self.ef_lo.value(), self.ef_hi.value(),
                               int(self.ef_pts.value()))
        ...
        def job():
            lo, hi = _rbm_api.reflux_band(prob, provider, r_hi=r_hi,
                                          n_scan=n_scan)
            at = None if lo is None else _rbm_api.analyse(
                prob, provider, r=lo * 1.02, EF=None)
    ...
    def _on_band_done(self, payload):
        ...
        self.status.setText(
            ...
            + (f", r_max = {hi:.4g}" if hi is not None else
               " (no maximum -- an ordinary column never breaks from more reflux)")
    ...
    def _on_region_done(self, payload):
        ...
        if reg["EF_min"] is not None:
            bits.append(f"minimum E/F ~ {reg['EF_min']:.3g} "
        ...
        if reg["operating"]:
            o = reg["operating"]
            bits.append(f"recommended E/F={o['EF']:.3g} "
    ...
    def _plot_current(self):
        ...
        if self._result is None:
            self._blank("Run Analyse to solve the pinches and build the bodies.")
            return
    ...
    def _draw_bodies(self, ax, res, base_alpha=0.30):
        ...
        * CONTRAST. Fill, outline and markers are drawn separately so only the
        ...
        for sec in res["sections"]:
            ...
            live = active.get(sec["name"], set())
            for i, body in enumerate(sec["bodies"]):
                V = np.asarray(body["vertices"], float)
                pts = np.unique(np.round(np.column_stack([V[:, lk], V[:, hk]]),
                                         12), axis=0)
                on = i in live
                ...
                if len(ring) >= 3:
                    ax.add_patch(Polygon(ring, closed=True, facecolor=colour,
                                         edgecolor="none",
                                         alpha=base_alpha if on
                                         else base_alpha * 0.4, zorder=1))
                    loop = np.vstack([ring, ring[:1]])
                    ax.plot(loop[:, 0], loop[:, 1], "-", color=colour,
                            lw=2.0 if on else 1.0, alpha=0.95 if on else 0.55,
                            zorder=2)
                elif len(ring) == 2:
                    ax.plot(ring[:, 0], ring[:, 1], "-", color=colour,
                            lw=2.4 if on else 1.2, alpha=0.95 if on else 0.55,
                            zorder=2)
                ax.plot(pts[:, 0], pts[:, 1], "o", color=colour,
                        ms=5.0 if on else 3.5, mfc=colour if on else "none",
                        mew=1.2, alpha=0.95 if on else 0.6, zorder=3)
            ax.plot([], [], "-", color=colour, lw=2,
                    label=f"{sec['name']} bodies")
    ...
    def _plot_ternary(self, res):
        ...
        for sec in res["sections"]:
            for p in sec["pinches"]:
                ...
                ax.plot(p["x"][lk], p["x"][hk], mark, color="k", ms=6, mfc="w",
                        mew=1.4, label=lbl if lbl not in seen else None)
                seen.add(lbl)
        ...
        verdict = "bodies intersect" if res["feasible"] else \
            f"bodies apart by {res['max_gap']:.3g}"
    ...
    def _plot_pinches(self, res):
        ...
        for sec in res["sections"]:
            ...
            for p in sec["pinches"]:
                ...
                x = p["x"]
                mark, lbl = PINCH_MARKER.get(p["kind"], PINCH_MARKER["?"])
                ax.plot(x[lk], x[hk], mark, color=colour, ms=8, mfc="w", mew=1.6,
                        label=lbl if lbl not in seen else None, zorder=5)
                seen.add(lbl)
                ...
                for k in range(p["eigvecs"].shape[1]):
                    ...
                    v = v / n * mags[k] * scale
                    ax.annotate(
                        "", xy=(x[lk] + v[lk], x[hk] + v[hk]), xytext=(x[lk], x[hk]),
                        arrowprops=dict(
                            arrowstyle="->", color=colour, lw=1.3,
                            linestyle="-" if mags[k] < 1.0 else "--", alpha=0.9))
        ...
        ax.set_xlabel(f"{comps[lk]} mole fraction (LK)")
        ax.set_ylabel(f"{comps[hk]} mole fraction (HK)")
        ax.set_title(f"Pinches, eigendirections and rectification bodies "
                     f"at r={res['r']:g}", fontsize=10)
        ax.legend(fontsize=7, loc="upper right")
        self.figure.tight_layout()
        self.canvas.draw()
    ...
    def _plot_region(self):
        ...
        if self._region is None:
            self._blank("Run 'Operating region' (or 'Compute r_min / r_max' in "
        ...
        if np.any(np.isfinite(hi)):
            ...
            ax.fill_between(EF[ok], lo[ok], hi[ok], color="tab:green", alpha=0.15,
                            label="feasible")
        if reg["EF_min"] is not None:
            ax.axvline(reg["EF_min"], color="0.4", ls="--", lw=1.2)
            ax.annotate(f"(E/F)min ~ {reg['EF_min']:.3g}",
                        (reg["EF_min"], reg["r_at_EF_min"]),
                        textcoords="offset points", xytext=(6, 8), fontsize=8)
        if reg["operating"]:
            o = reg["operating"]
            ax.plot([o["EF"]], [0.5 * (o["r_min"] + o["r_max"])], "k*", ms=13,
                    label="recommended operating point")
        ax.plot([self.ef_spin.value()], [self.r_spin.value()], "kP", ms=10,
    ...
    def _fill_table(self, res):
        ...
        for i, (name, p) in enumerate(rows):
            mags = ("-" if p["eigvals"] is None else
                    ", ".join(f"{m:.3g}" for m in np.abs(p["eigvals"])))
            cells = [name,
...
def _demo():
    ...
    res = _rbm_api.analyse(prob, provider, r=float(w.r_spin.value()), EF=None)
    w._on_analyse_done(res)
    assert w._result is not None
    assert w.data_table.rowCount() == sum(len(s["pinches"])
    ...
    w._on_band_done({"band": (lo, hi),
                     "at": None if lo is None else
                     _rbm_api.analyse(prob, provider, r=lo * 1.02, EF=None)})
    if lo is not None:
        assert w.view_combo.currentIndex() == _VIEW_TERNARY, \
    ...
    stop = _drv.operating_region(
        prob, provider, EF_grid=np.linspace(0.4, 1.6, 4), r_hi=4.0, n_scan=6,
    ...
    print(f"rbm_module self-check OK  {w.data_table.rowCount()} pinches tabulated, "
