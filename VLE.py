...
def rectVLE(X, y, P, comps, props,  opts):
    NComps = len(comps)
    x = np.matrix(Y[0:NComps])
    T = X[NComps]
    Psat = antoineCalc(T,comps,props,opts.antMethod)
    ...
    for i in range(0,NComps):
        raoult[i,:] = x[i]*gamma[i]*Psat[i] - y[i]*phi[i]*P
        # dalton[i] = x[i]*gamma[i]*Psat[i]/phi[i]

    tray = np.append(raoult,[[np.sum(np.abs(x)) - float(1.00), np.sum(x) - float(1.00)]])
